...
def test_deve_adicionar_nova_pergunta(client, usuario_admin, criar_dados_de_teste, tipo_resposta):
    ...
    resultado = client.execute(adicionar_nova_pergunta_mutation, variables={
                               'temaId': tema_id, 'referenciaId': referencia_id, 'tipoResposta': tipo_resposta}, context_value=UsuarioEmContexto(usuario=usuario_admin))

    pergunta_mais_nova = Pergunta.objects.last()
    assert resultado == {'data': OrderedDict([('cadastrarPergunta', {'pergunta': {'id': f'{pergunta_mais_nova.id}', 'tema': {'nome': f'{pergunta_mais_nova.tema.nome}'},
                                             'enunciado': f'{pergunta_mais_nova.enunciado}', 'tipoResposta': f'{pergunta_mais_nova.tipo_resposta}', 'status': pergunta_mais_nova.status, 'revisadoPor': None}})])}
...
def test_deve_editar_nova_pergunta(client, usuario_admin, criar_dados_de_teste):
    ...
    resultado = client.execute(editar_pergunta_mutation, variables={
                               'temaId': tema_id, 'referenciaId': referencia_id, 'perguntaId': pergunta_id}, context_value=UsuarioEmContexto(usuario=usuario_admin))
    nova_pergunta.refresh_from_db()

    assert resultado == {'data': OrderedDict([('editarPergunta', {'pergunta': {
                                             'id': f'{nova_pergunta.id}', 'enunciado': 'Novo enunciado', 'revisadoPor': None, 'status': True}})])}
...
def test_deve_retornar_pergunta_aleatoria_de_um_tema(client, criar_dados_de_teste, usuario_admin):
    ...
    resultado = client.execute(pergunta_aleatoria_query, variables={
                               'temaId': tema_id}, context_value=UsuarioEmContexto(usuario=usuario_admin))

    assert resultado == {'data': {'pergunta': [
        {'id': f'{pergunta_conhecimentos_gerais.id}', 'enunciado': f'{pergunta_conhecimentos_gerais.enunciado}'}]}}
    assert 'errors' not in resultado
